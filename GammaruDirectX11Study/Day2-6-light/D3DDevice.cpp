...
bool InitD3D(HWND hwnd)
{
    ...
    for (int i = 0; i < 256; ++i)
        for (int j = 0; j < 256; ++j) {
            arr[i][j][0] = i / 256.0f;
            arr[i][j][1] = j / 256.0f;
            arr[i][j][2] = ((i - 128.0f) * (i - 128.0f) + (j - 128.0f) * (j - 128.0f)) / (128.0f * 128.0f * 2.0f);
        }
...
void ClearD3D()
{
    ...
    if (g_pVSConstBufferPerScene != NULL) g_pVSConstBufferPerScene->Release();
...
void Render()
{
    ...
    vsConstBufferPerFrame.camPos = XMFLOAT3(camPos[0], camPos[1], camPos[2]);
